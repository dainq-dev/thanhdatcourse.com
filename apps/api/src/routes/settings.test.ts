...
describe("Settings Routes", () => {
  ...
  beforeAll(async () => {
    ...
    const userRes = await app.request("/api/auth/register", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "Settings User", email: "settingsuser@example.com", password: "password123", confirmPassword: "password123" }),
    });
  ...
  describe("PUT /api/settings/batch", () => {
    ...
    test("with admin token updates keys and returns { updated, keys }", async () => {
      const res = await app.request("/api/settings/batch", {
        ...
        body: JSON.stringify({ site_title: "Updated Title", theme_color: "#FF0000" }),
      });
      expect(res.status).toBe(200);
      const data = await res.json();
    ...
    test("value actually changed in DB", async () => {
      const [row] = await db.select().from(siteSettings).where(eq(siteSettings.key, "site_title"));
    ...
    test("creates a new key (upsert behavior)", async () => {
      ...
      const [row] = await db.select().from(siteSettings).where(eq(siteSettings.key, "brand_new_test_key"));
      expect(row).toBeDefined();
      expect(row.value).toBe("hello world");

      await db.delete(siteSettings).where(eq(siteSettings.key, "brand_new_test_key"));
    });
