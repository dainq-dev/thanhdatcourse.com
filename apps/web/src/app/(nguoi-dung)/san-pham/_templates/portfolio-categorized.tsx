...
function getUniqueCategories(items: PortfolioItem[]): string[] {
...
export function PortfolioCategorized({
  settings,
  portfolios,
  ctaItems,
  engine,
  ...
  const categories = getUniqueCategories(portfolios);
    ...
    <>
      ...
      {categories.length > 1 && (
        <section className={styles.categoryFilter}>
          {categories.map((cat) => (
            <button key={cat} type="button" className={styles.filterBtn}>
              {cat}
            </button>
          ))}
        </section>
