...
import { getDefaultData, useBlockEditorState } from "./editorState";
...
export function BlockEditor({
  ...
  const handleDrop = useCallback(
    (type: Block["type"], atIndex?: number) => {
  ...
  const handleDuplicate = useCallback(() => {
    if (selectedBlock) {
      const copy: Block = {
        ...
        data: JSON.parse(JSON.stringify(selectedBlock.data)),
      } as Block;
  ...
  const handleMove = useCallback(
    (dir: "up" | "down") => {
      ...
      if (dir === "up" && idx > 0) editor.reorderBlocks(idx, idx - 1);
        ...
        editor.reorderBlocks(idx, idx + 1);
    },
    ...
    <div className={styles.workspace}>
      <div className={styles.topBar}>
        ...
        <div className={styles.topBarRight}>
          ...
          <button
            type="button"
            className={`${styles.topBarBtn} ${!showRightPanel ? styles.active : ""}`}
            onClick={() => setShowRightPanel(!showRightPanel)}
            title="Toggle cấu hình panel"
          >
            <Settings2 size={17} />
          </button>
          {onPreview && (
          ...
          {onPublish && (
            <button
              type="button"
              className={styles.topBarPublish}
              onClick={onPublish}
              disabled={saving}
            >
              Xuất bản
            ...
            {blocks.length === 0 ? (
              <div className={styles.dropZone}>
                ...
                <div className={styles.dropZoneHint}>
                  hoặc bấm vào block bên panel Components để thêm
                </div>
              </div>
            ) : (
              <SortableBlockList
        ...
        {showRightPanel && (
          <>
            ...
            >
              <RightPanel
                block={selectedBlock}
                onChange={(id, data) => editor.updateBlock(id, data)}
...
function SortableBlockList({
  ...
  onDrop: (t: Block["type"], i?: number) => void;
...
function SortableBlockItem({
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <div
        className={`${styles.blockItem} ${isSelected ? styles.blockItemSelected : ""}`}
        onClick={(e) => {
          e.stopPropagation();
          onSelect();
        }}
      >
        ...
        <div className={styles.blockItemActions}>
          <button
            type="button"
            className={styles.blockItemActionBtn}
            onClick={(e) => {
              e.stopPropagation();
              onDelete();
            }}
