...
export function GalleryBlock({ data }: {
  data: {
    ...
    columns: number; gap: string; layout: string;
    rounded: string; shadow: string; hoverZoom: boolean; lightbox: boolean;
  };
}) {
  const d = data;
  const images = d.images || [];
  if (images.length === 0) return <div style={{ padding: "2rem", textAlign: "center", color: "#94A3B8" }}>Chưa có ảnh nào</div>;

  return (
    <div className={styles.root}>
      <div className={`${styles.grid} ${d.layout === "masonry" ? styles.masonry : ""}`}
        style={{
          gridTemplateColumns: d.layout !== "masonry" ? `repeat(${d.columns || 3}, 1fr)` : undefined,
        ...
        {images.map((img, i) => (
          <div key={i} className={`${styles.item} ${d.hoverZoom ? styles.hoverZoom : ""}`}
            ...
            <img
              ...
              onClick={d.lightbox ? () => window.open(resolveMediaUrl(img.mediaId, "full"), "_blank") : undefined}
            />
