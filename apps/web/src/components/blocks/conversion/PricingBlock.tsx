  ...
  return (
    <div className={`${styles.root} ${isVertical ? styles.vertical : ""}`}>
      {plans.map((plan: any, i: number) => (
        <div key={i} className={`${styles.plan} ${plan.highlighted ? styles.highlighted : ""}`}>
          ...
          {plan.description && <div className={styles.desc}>{plan.description}</div>}
          <ul className={styles.features}>
            {plan.features.map((f: string, fi: number) => <li key={fi}>{f}</li>)}
          </ul>
          <a href={plan.cta.url} className={styles.cta}>{plan.cta.text}</a>
        </div>
      ))}
    </div>
  );
