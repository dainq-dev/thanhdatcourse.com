...
function detectMime(header: Uint8Array, fileName: string): string | null {
  ...
  if (
    ...
    header[8]  === AVIF_FTYP[4] && header[9]  === AVIF_FTYP[5] &&
    header[10] === AVIF_FTYP[6] && header[11] === AVIF_FTYP[7]
  ) {
    return "image/avif";
  }
  ...
  if (
    header.length >= 12 &&
    header[4] === MP4_FTYP[0] && header[5] === MP4_FTYP[1] &&
    header[6] === MP4_FTYP[2] && header[7] === MP4_FTYP[3]
  ) {
    return "video/mp4";
  }
...
function getCategory(mimeType: string): string | null {
  ...
  if (mimeType.includes("officedocument") || mimeType.includes("opendocument")) return "document";
  return null;
}
