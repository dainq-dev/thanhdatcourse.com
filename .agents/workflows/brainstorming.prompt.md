...
> **If the problem involves a tricky bug, complex debugging, or root cause analysis**, use the `strategic-problem-solving` skill for structured frameworks (5W, 5 Whys, Ishikawa, First Principles).

> **If the problem requires understanding unfamiliar code or architecture**, use the `strategic-codebase-navigation` skill to map the relevant codebase before brainstorming solutions.
...
- **Backend Engineer**: Is this technically feasible? What are the constraints? (Reference `backend-dev` skill)
- **Frontend Engineer**: Is the UI feasible? What component patterns apply? (Reference `frontend-dev` skill)
- **Infra Engineer**: Can we deploy and scale this? (Reference `infra-dev` skill)
...
This document can feed directly into:

- `/quality-review` to challenge and stress-test the top ideas before committing
- `/bdd-spec` to write behavior specs for the top ideas