...
Root folder: .agents/skills/gsap

## Repo structure

- **skills/gsap/** — Each subdirectory is one skill. The CLI and agents discover skills by scanning `skills/gsap/*` for directories that contain `SKILL.md`.
- **Skill directory name** must exactly match the `name` in that skill’s frontmatter (e.g. `skills/gsap/gsap-core/` ↔ `name: gsap-core`).
